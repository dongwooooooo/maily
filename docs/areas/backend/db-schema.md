...
| version | int | not null default 0 | 상태 변경마다 증가. `item:{id}:state:{version}` idempotency key 구성과 no-op 갱신 판별에 쓴다. |
...
| created_at | timestamptz | not null | 알림 발생 시각 — 목록 최신순 정렬 기준(`outbox_events.created_at`과 동일 패턴). |
