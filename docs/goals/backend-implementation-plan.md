...
publish durable events through C0 outbox/job dispatch. Gmail read/sync is isolated behind
D3 `GmailReaderPort`, Gmail write/mutation is isolated behind D6 `GmailMutationPort`, and
all Gmail writes go through the D6 command ledger.
...
- 모듈 경계는 `docs/areas/backend/module-boundaries.md`의 D1-D8과 구현 유닛을 따른다.
...
- Gmail mutation은 D6 command ledger, activity log, undo 가능 여부 없이 구현하지 않는다.
- 모듈 간 side effect는 직접 도메인 service 호출이 아니라 `outbox_events`, `job_runs`,
  idempotency key, read model로 연결한다.
- C0 Backend Core는 도메인 모듈이 아니라 transaction, outbox, idempotency, job dispatch
  같은 공통 실행 메커니즘만 제공한다.
- D2 외부 모듈은 OAuth token을 직접 읽지 않는다.
- D3 Gmail read/sync는 `GmailReaderPort`, D6 Gmail write/mutation은 `GmailMutationPort`만 사용한다.
- D4 briefing projection은 재생성 가능해야 한다. 사용자 item state는 projection과 분리한다.
...
| 도메인 | 주요 위치 | 내부 구성 |
|---|---|---|
| C0 Backend Core | `app/core/`, `app/core/jobs/`, `app/api/`, `app/db/` | config, database, redis, outbox, idempotency, logging, job dispatcher/lock/retry/registry |
| D1 Identity & Workspace | `app/domains/identity/` | models, schemas, repository, service, router |
| D2 Connected Gmail Sources | `app/domains/mail_sources/` | models, schemas, repository, service, router, credentials, oauth, events, purge, `jobs/` |
| D3 Gmail Intake & Snapshot | `app/domains/mail_intake/` | models, schemas, repository, service, router, gmail_reader, fake_reader, live_reader, events, purge, `jobs/` |
| D4 Briefing & Item State | `app/domains/briefing/` | models, schemas, repository, service, router, item_state, reminders, events, purge, `jobs/` |
| D5 Labels & Classification | `app/domains/labels/` | models, schemas, repository, service, router, events |
| D6 Gmail Actions & Activity | `app/domains/gmail_actions/` | models, schemas, repository, service, router, gmail_mutator, fake_mutator, live_mutator, activity, undo, events, purge, `jobs/` |
| D7 Assistant Decisions | `app/domains/assistant_decisions/` | models, schemas, repository, service, router, summaries, rules, cleanup, llm, fake_llm, events, purge, `jobs/` |
| D8 Notifications & Recovery | `app/domains/notifications/` | models, schemas, repository, service, router, events, `jobs/` |
...
| IG1 | Live Gmail Watch Integration | D2, D3, infra | 테스트 Gmail 계정의 새 메일이 Pub/Sub/history 경로로 반영된다. |

## Task 1: Backend Core Runtime

**Context:** C0 Backend Core  
...
**Gate:** D1 request context is available to every authenticated API.

## Task 3: Connected Gmail Sources

**Context:** D2 Connected Gmail Sources  
...
**Context:** D3 Gmail Intake & Snapshot  
...
**Context:** D4 Briefing & Item State  
...
**Context:** D5 Labels & Classification  
...
- [ ] Write message move test that records correction signal and requests D6 label apply command.
...
**Context:** D6 Gmail Actions & Activity  
...
- [ ] Write boundary test that D6 uses `GmailMutationPort` and never reads OAuth token directly.
- [ ] Write negative boundary test that D6 does not import `mail_intake.gmail_reader`.
...
## Task 10: Summary Privacy

**Context:** D7 Assistant Decisions  
**Goal:** 계정별 요약 설정과 최소 payload 요약 계약을 구현한다.
...
- [ ] Add migrations for `summary_jobs`, `message_summaries`.
- [ ] Implement fake LLM client and summary worker.
- [ ] Run `cd development/backend && python -m pytest tests/domains/assistant_decisions/test_summary_privacy.py tests/domains/assistant_decisions/test_generate_summary_job.py -q`.

**Gate:** G6 privacy contract passes.
...
- [ ] Write approval test that requests D6 command and does not call Gmail directly.
...
**Context:** D8 Notifications & Recovery  
...
- [ ] Write recovery view test where account/sync source state comes from D2/D3 and D8 stores view data only.
...
**Context:** D2 orchestrated, D3/D4/D6/D7 participants  
...
**Context:** D2, D3, infra integration  
...
**Context:** C0 plus all backend modules  
...
|---|---|---|---|
| Google OAuth client/redirect 설정 | live 계정 연결 지연 | mocked Google profile, fake credential repository | 서비스 로그인 계정과 연결 Gmail 계정은 분리 |
| Pub/Sub topic/webhook 설정 | live watch POC 지연 | fake notification과 fallback polling TDD | 지속 sync는 수동 refresh API만으로 대체하지 않음 |
...
| Gmail write/Undo 역연산 | write action 출시 지연 | fake GmailMutationPort, command ledger, activity schema | command/activity/undo 없이 Gmail mutation 금지 |
| LLM provider 결정 | 요약 출시 지연 | fake LLM client, privacy test | raw prompt/body 저장 금지 |
