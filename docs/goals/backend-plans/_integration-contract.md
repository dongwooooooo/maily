...
- core의 app factory는 `app/domains/` 아래를 순회하며 위 심볼을 수집한다. 도메인 추가 시 core 코드 수정 없이 등록된다.
- registry.py는 수집된 `JOB_HANDLERS`를 합쳐 job_type→handler 맵을 만든다. 중복 job_type은 부팅 시 에러.
